...
assert_valid_trial_config <- function(trial_metadata) {
  ...
  if (length(missing_fields) > 0) {
    stop(paste(
      "Missing required fields in trial_metadata:",
  ...
  for (field in required_fields) {
    if (
      is.null(trial_metadata[[field]]) ||
        all(trial_metadata[[field]] == "")
    ) {
      stop(paste(
        "Missing value for required fields in trial_metadata:",
        field,
        "cannot be NULL and must be a non-empty string."
      ))
    }
  }
  invisible(trial_metadata)
}
