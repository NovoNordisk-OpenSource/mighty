#' Build parameters for `mighty_col_mutate` and `mighty_col_rename` templates
#'
#' Both `mighty_col_mutate` and `mighty_col_rename` share the same three
#' parameters, so this single function serves both. The templates differ only in
#' the generated R verb: `mighty_col_mutate` uses `dplyr::mutate()` (keeps the
#' source column) while `mighty_col_rename` uses `dplyr::rename()` (replaces it
#' in-place).
#'
