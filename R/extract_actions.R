extract_actions <- function(ui_table) {
  ...
  if (any(!is.na(ui_table$code_id))) {
    actions_with_code_id <- ui_table |>
      ...
      dplyr::summarise(outputs = list(unlist(outputs)),
  ...
  if (any(is.na(ui_table$code_id))) {
    actions_no_code_id <- ui_table |>
      dplyr::filter(is.na(code_id)) |>
      dplyr::select(domain, code_id, parameters, outputs, id, depend_cols, depend_rows)
  }

  # Collect all actions and add action id
  actions_combined <- dplyr::bind_rows(actions_with_code_id,
                                       actions_no_code_id) |>
    as.data.table() |>
    add_node_id() |>
    update_depend_rows() |>
