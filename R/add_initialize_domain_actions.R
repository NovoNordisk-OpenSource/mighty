add_initialize_domain_actions <- function(actions, ui_init) {
  ...
  col_copy_outputs <- split(actions[actions$type == "col_copy", c("domain", "outputs")], by = "domain")

  has_domain_filter <- lapply(purrr::list_transpose(ui_init)$filter_domain, function(x) {
    any(!is.na(unlist(x)))
  })
