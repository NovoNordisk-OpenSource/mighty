...
make_edges <- function(nodes, primary_domain = "ADSL") {
  ...
  all_edges <- combine_and_clean_edges(
...
create_column_dependency_edges <- function(nodes) {
  ...
  node_id_no_dep <- lapply(seq_len(nrow(nodes2)), function(i) {
    if (nrow(nodes2$depend_cols[[i]]) == 0) {
      nodes2$node_id[[i]]
    }
  }) |>
    unlist()
...
expand_child_columns <- function(nodes) {
  ...
  extract_child_columns <- function(x) {
    data.table::data.table(
      ...
      child_column_domain_type = ifelse(
        x$type == "preprocess_domain",
        "init",
        "adam"
      )
    )
  ...
  children_expanded <- nodes[
    nodes$type != "row_compute",
...
create_row_dependency_edges <- function(nodes) {
  ...
  new_row_edges1 <-
    if (any(!is.na(nodes$depend_rows))) {
      nodes[
        !is.na(nodes$depend_rows),
        unlist(get("depend_rows")),
        by = list(node_id = get("node_id"), domain = get("domain"))
      ] |>
  ...
  row_actions_parents <- expand_parent_columns(nodes[
    nodes$type == "row_compute"
  ])
  config_actions_children <- expand_child_columns(nodes[
    nodes$type %in% c("init_domain", "filter_domain")
  ])

  # Identify edges through a merge
  new_row_edges2 <- merge(
    config_actions_children[, list(
      node_id = get("node_id"),
      child_column = get("child_column"),
      child_column_domain = get("child_column_domain")
    )],
    row_actions_parents[, list(
      node_id = get("node_id"),
      parent_column = get("parent_column"),
      parent_column_domain = get("parent_column_domain")
    )],
  ...
  setnames(
    new_row_edges2,
    c("child_column", "domain", "parent_node", "node_id")
  )
    ...
    if (nrow(filter_action) > 0) {
      ...
      children_to_remove <- lapply(seq_len(nrow(other_actions)), function(i) {
        is_child_to_remove <- other_actions$type[[i]] != "col_copy" &
          other_actions$outputs[[i]] %in% filter_action_outputs |> any()
