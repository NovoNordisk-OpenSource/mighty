...
format_error_message <- function(error_row) {
  ...
  enhanced_messages <- list(
    ...
    additionalProperties = paste(
      "Unexpected field '{params$additionalProperty}' found.",
    ...
    minLength = paste(
      "String must be at least {params$limit} characters long.",
      "Current length: {nchar(as.character(error_row$data))}."
    ),
...
format_error_path <- function(path, error_row = NULL) {
  base <- if (is.na(path) || path == "") {
    "Root level"
  } else {
    enhance_path_readability(path, error_row)
  }
...
enhance_array_indices <- function(path, error_row) {
  ...
  if (match_data[1] != -1) {
    for (i in rev(seq_along(match_data))) {
      ...
      identifier <- find_array_element_identifier(
        error_row,
        array_name,
        array_index
      )
  ...
  if ("data" %in% names(error_row) && is.list(error_row$data)) {
    ...
    for (field in identifier_fields) {
      if (field %in% names(data_obj)) {
        ...
        if (
          !is.null(value) && !is.na(value) && nchar(as.character(value)) > 0
        ) {
          return(as.character(value))
        }
      }
