test_that("data_context provides metadata from sdtm.dm testdata", {
  ...
  setup_testdata(testdata = "pharmaverse", test_data_path = output_path, sdtm_domains = c("dm"))
...
test_that("data_context provides metadata from sdtm and adam", {
  ...
  # Test default table names (datasource = "sdtm")
  expect_equal(sort(names(dc$get_table_names())), c("sdtm.ae", "sdtm.dm", "sdtm.dm_vaccine", "sdtm.lb", "sdtm.suppdm", "sdtm.sv"))
  expect_equal(sort(as.character(dc$get_table_names())), c("ae", "dm", "dm_vaccine", "lb", "suppdm", "sv"))

  # Test table names in both SDTM and ADAM
  expect_equal(sort(names(dc$get_table_names(c("sdtm", "adam")))), c("adam.adsl", "sdtm.ae", "sdtm.dm", "sdtm.dm_vaccine", "sdtm.lb", "sdtm.suppdm", "sdtm.sv"))
  expect_equal(sort(as.character(dc$get_table_names(c("sdtm", "adam")))), c("adsl", "ae", "dm", "dm_vaccine", "lb", "suppdm", "sv"))

  expect_equal(dc$get_table_names(datasources = c("adam"), prefix_datasource = TRUE)[[1]], c("adam.adsl"))
  expect_equal(dc$get_table_names(datasources = c("adam"), prefix_datasource = TRUE)[["adam.adsl"]], c("adam.adsl"))
  expect_equal(dc$get_table_names(datasources = c("adam"), prefix_datasource = FALSE)[["adam.adsl"]], c("adsl"))

  expect_true(dc$has_variables(table_name = "dm", variable_name = c("STUDYID", "USUBJID")))
  expect_false(dc$has_variables(table_name = "dm", variable_name = "NONEXISTING"))
  expect_true(dc$has_variables(datasource = "metadata", table_name = "mdvisit", variable_name = "STUDYID"))

  expect_equal(dc$get_table_variables(table_name = "sv"),
...
test_that("data_context provide column metadata", {
  ...
  # SETUP -------------------------------------------------------------------
  output_path <- withr::local_tempdir()
  setup_testdata(testdata = "pharmaverse", sdtm_domains = ("dm"), adam_domains = c("adsl"), test_data_path = output_path)
  cnt <- connector::connect(file.path(output_path, "_connector.yml"))
