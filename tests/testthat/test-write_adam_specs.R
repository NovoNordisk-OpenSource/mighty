test_that("Simple yaml specs are semantically identical", {
  ...
  yaml_object_now_normalised <- normalise_yaml_structure(
    yaml_object_non_normalised
  )
...
test_that("Normalise is working as expected", {
  ...
  expect_equal(
    normalised_yaml_object$table_metadata,
    yaml_object_normalised$table_metadata
  )
  expect_equal(normalised_yaml_object$init, yaml_object_normalised$init)
  expect_equal(
    normalised_yaml_object$column_metadata,
    yaml_object_normalised$column_metadata
  )
})
...
test_that("Reading adam specs and writing back produces the same result", {
  ...
  compare <- waldo::compare(
    yaml_expect_adsl_normalised,
    yaml_actual_adsl_normalised
  )
  # expect file contents are identical
  expect_length(compare, 0)

  compare <- waldo::compare(
    yaml_expect_adlb_normalised,
    yaml_actual_adlb_normalised
  )
  # expect file contents are identical
  expect_length(compare, 0)
})
